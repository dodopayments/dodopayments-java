...
internal class WebhookCreateParamsTest {
    ...
    fun body() {
      ...
      assertThat(body.filterTypes().getOrNull()).containsExactly(WebhookEventType.PAYMENT_SUCCEEDED)
      assertThat(body.headers()).contains(WebhookCreateParams.Headers.builder()
          .putAdditionalProperty("foo", JsonValue.from("string"))
          .build())
      assertThat(body.idempotencyKey()).contains("idempotency_key")
      assertThat(body.metadata()).contains(WebhookCreateParams.Metadata.builder()
          .putAdditionalProperty("foo", JsonValue.from("string"))
          .build())
