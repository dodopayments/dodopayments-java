...
class AddonResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(addonResponse: AddonResponse) =
            apply {
                id = addonResponse.id
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (businessId.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (currency.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (price.asKnown().isPresent) 1 else 0) + (taxCategory.asKnown().getOrNull()?.validity() ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0) + (if (image.asKnown().isPresent) 1 else 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is AddonResponse && id == other.id && businessId == other.businessId && createdAt == other.createdAt && currency == other.currency && name == other.name && price == other.price && taxCategory == other.taxCategory && updatedAt == other.updatedAt && description == other.description && image == other.image && additionalProperties == other.additionalProperties
    }
    ...
    override fun toString() = "AddonResponse{id=$id, businessId=$businessId, createdAt=$createdAt, currency=$currency, name=$name, price=$price, taxCategory=$taxCategory, updatedAt=$updatedAt, description=$description, image=$image, additionalProperties=$additionalProperties}"
}
