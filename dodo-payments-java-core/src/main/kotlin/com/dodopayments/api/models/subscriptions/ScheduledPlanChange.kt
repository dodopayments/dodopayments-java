...
class ScheduledPlanChange @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(scheduledPlanChange: ScheduledPlanChange) =
            apply {
     ...
     *   expected type.
     */
    fun validate(): ScheduledPlanChange =
        apply {
            if (validated) {
              return@apply
            }
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (addons.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (effectiveAt.asKnown().isPresent) 1 else 0) + (if (productId.asKnown().isPresent) 1 else 0) + (if (quantity.asKnown().isPresent) 1 else 0) + (if (productDescription.asKnown().isPresent) 1 else 0) + (if (productName.asKnown().isPresent) 1 else 0)
    ...
    class Addon @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (addonId.asKnown().isPresent) 1 else 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (quantity.asKnown().isPresent) 1 else 0)
        ...
        override fun equals(other: Any?): Boolean {
          ...
          return other is Addon && addonId == other.addonId && name == other.name && quantity == other.quantity && additionalProperties == other.additionalProperties
        }

        private val hashCode: Int by lazy { Objects.hash(addonId, name, quantity, additionalProperties) }

        override fun hashCode(): Int = hashCode

        override fun toString() = "Addon{addonId=$addonId, name=$name, quantity=$quantity, additionalProperties=$additionalProperties}"
    }
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is ScheduledPlanChange && id == other.id && addons == other.addons && createdAt == other.createdAt && effectiveAt == other.effectiveAt && productId == other.productId && quantity == other.quantity && productDescription == other.productDescription && productName == other.productName && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(id, addons, createdAt, effectiveAt, productId, quantity, productDescription, productName, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "ScheduledPlanChange{id=$id, addons=$addons, createdAt=$createdAt, effectiveAt=$effectiveAt, productId=$productId, quantity=$quantity, productDescription=$productDescription, productName=$productName, additionalProperties=$additionalProperties}"
}
