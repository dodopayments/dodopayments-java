...
class CreditBalanceLowWebhookEvent @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(creditBalanceLowWebhookEvent: CreditBalanceLowWebhookEvent) =
            apply {
     ...
     *   expected type.
     */
    fun validate(): CreditBalanceLowWebhookEvent =
        apply {
            if (validated) {
              return@apply
            }
    ...
    internal fun validity(): Int =
        (if (businessId.asKnown().isPresent) 1 else 0) + (data.asKnown().getOrNull()?.validity() ?: 0) + (if (timestamp.asKnown().isPresent) 1 else 0) + type.let {
            if (it == JsonValue.from("credit.balance_low")) 1 else 0
        }
    ...
    class Data @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (availableBalance.asKnown().isPresent) 1 else 0) + (if (creditEntitlementId.asKnown().isPresent) 1 else 0) + (if (creditEntitlementName.asKnown().isPresent) 1 else 0) + (if (customerId.asKnown().isPresent) 1 else 0) + (if (subscriptionCreditsAmount.asKnown().isPresent) 1 else 0) + (if (subscriptionId.asKnown().isPresent) 1 else 0) + (if (thresholdAmount.asKnown().isPresent) 1 else 0) + (if (thresholdPercent.asKnown().isPresent) 1 else 0)
        ...
        override fun equals(other: Any?): Boolean {
          ...
          return other is Data && availableBalance == other.availableBalance && creditEntitlementId == other.creditEntitlementId && creditEntitlementName == other.creditEntitlementName && customerId == other.customerId && subscriptionCreditsAmount == other.subscriptionCreditsAmount && subscriptionId == other.subscriptionId && thresholdAmount == other.thresholdAmount && thresholdPercent == other.thresholdPercent && additionalProperties == other.additionalProperties
        }

        private val hashCode: Int by lazy { Objects.hash(availableBalance, creditEntitlementId, creditEntitlementName, customerId, subscriptionCreditsAmount, subscriptionId, thresholdAmount, thresholdPercent, additionalProperties) }

        override fun hashCode(): Int = hashCode

        override fun toString() = "Data{availableBalance=$availableBalance, creditEntitlementId=$creditEntitlementId, creditEntitlementName=$creditEntitlementName, customerId=$customerId, subscriptionCreditsAmount=$subscriptionCreditsAmount, subscriptionId=$subscriptionId, thresholdAmount=$thresholdAmount, thresholdPercent=$thresholdPercent, additionalProperties=$additionalProperties}"
    }
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is CreditBalanceLowWebhookEvent && businessId == other.businessId && data == other.data && timestamp == other.timestamp && type == other.type && additionalProperties == other.additionalProperties
    }
    ...
    override fun toString() = "CreditBalanceLowWebhookEvent{businessId=$businessId, data=$data, timestamp=$timestamp, type=$type, additionalProperties=$additionalProperties}"
}
