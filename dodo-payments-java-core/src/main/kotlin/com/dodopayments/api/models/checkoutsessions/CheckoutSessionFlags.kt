...
private constructor(
     ...
     *   the server responded with an unexpected value).
     */
    fun allowEditingAddons(): Optional<Boolean> =
        allowEditingAddons.getOptional("allow_editing_addons")
     ...
     *   the server responded with an unexpected value).
     */
    fun requireCardholderName(): Optional<Boolean> =
        requireCardholderName.getOptional("require_cardholder_name")
     ...
     *   the server responded with an unexpected value).
     */
    fun requireTaxId(): Optional<Boolean> = requireTaxId.getOptional("require_tax_id")
     ...
     *   the server responded with an unexpected value).
     */
    fun singlePage(): Optional<Boolean> = singlePage.getOptional("single_page")
    ...
    internal fun validity(): Int =
        (if (allowCurrencySelection.asKnown().isPresent) 1 else 0) +
            ...
            (if (allowEditingAddons.asKnown().isPresent) 1 else 0) +
            ...
            (if (requireCardholderName.asKnown().isPresent) 1 else 0) +
            (if (requirePhoneNumber.asKnown().isPresent) 1 else 0) +
            (if (requireTaxId.asKnown().isPresent) 1 else 0) +
            (if (singlePage.asKnown().isPresent) 1 else 0)
