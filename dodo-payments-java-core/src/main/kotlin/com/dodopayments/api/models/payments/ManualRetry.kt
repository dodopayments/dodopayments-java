...
private constructor(
     ...
     */
    fun retryAvailableAt(): Optional<OffsetDateTime> =
        retryAvailableAt.getOptional("retry_available_at")
    ...
    class Builder internal constructor() {
        ...
        /** Alias for calling [Builder.retryAvailableAt] with `retryAvailableAt.orElse(null)`. */
        fun retryAvailableAt(retryAvailableAt: Optional<OffsetDateTime>) =
            retryAvailableAt(retryAvailableAt.getOrNull())
    ...
    internal fun validity(): Int =
        (if (invoiceId.asKnown().isPresent) 1 else 0) +
            (if (isManualRetry.asKnown().isPresent) 1 else 0) +
            (if (paymentId.asKnown().isPresent) 1 else 0) +
            (if (retryAttempt.asKnown().isPresent) 1 else 0) +
            (if (sendsAllowed.asKnown().isPresent) 1 else 0) +
            (if (sendsUsed.asKnown().isPresent) 1 else 0) +
            (if (retryAvailableAt.asKnown().isPresent) 1 else 0) +
