...
class Event @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (businessId.asKnown().isPresent) 1 else 0) + (if (customerId.asKnown().isPresent) 1 else 0) + (if (eventId.asKnown().isPresent) 1 else 0) + (if (eventName.asKnown().isPresent) 1 else 0) + (if (timestamp.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is Event && businessId == other.businessId && customerId == other.customerId && eventId == other.eventId && eventName == other.eventName && timestamp == other.timestamp && metadata == other.metadata && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(businessId, customerId, eventId, eventName, timestamp, metadata, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "Event{businessId=$businessId, customerId=$customerId, eventId=$eventId, eventName=$eventName, timestamp=$timestamp, metadata=$metadata, additionalProperties=$additionalProperties}"
}
