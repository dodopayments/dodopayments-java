...
class CustomerCreateParams private constructor(
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (email.asKnown().isPresent) 1 else 0) + (if (name.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (phoneNumber.asKnown().isPresent) 1 else 0)
        ...
        override fun equals(other: Any?): Boolean {
          ...
          return other is Body && email == other.email && name == other.name && metadata == other.metadata && phoneNumber == other.phoneNumber && additionalProperties == other.additionalProperties
        }

        private val hashCode: Int by lazy { Objects.hash(email, name, metadata, phoneNumber, additionalProperties) }

        override fun hashCode(): Int = hashCode

        override fun toString() = "Body{email=$email, name=$name, metadata=$metadata, phoneNumber=$phoneNumber, additionalProperties=$additionalProperties}"
    }
