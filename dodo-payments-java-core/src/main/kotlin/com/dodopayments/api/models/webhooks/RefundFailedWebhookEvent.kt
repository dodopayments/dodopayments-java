...
class RefundFailedWebhookEvent @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(refundFailedWebhookEvent: RefundFailedWebhookEvent) =
            apply {
     ...
     *   expected type.
     */
    fun validate(): RefundFailedWebhookEvent =
        apply {
            if (validated) {
              return@apply
            }
    ...
    internal fun validity(): Int =
        (if (businessId.asKnown().isPresent) 1 else 0) + (data.asKnown().getOrNull()?.validity() ?: 0) + (if (timestamp.asKnown().isPresent) 1 else 0) + type.let {
            if (it == JsonValue.from("refund.failed")) 1 else 0
        }
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is RefundFailedWebhookEvent && businessId == other.businessId && data == other.data && timestamp == other.timestamp && type == other.type && additionalProperties == other.additionalProperties
    }
    ...
    override fun toString() = "RefundFailedWebhookEvent{businessId=$businessId, data=$data, timestamp=$timestamp, type=$type, additionalProperties=$additionalProperties}"
}
