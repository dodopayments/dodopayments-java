...
class CustomerRequest private constructor(
    ...
    fun attachExistingCustomer(): Optional<AttachExistingCustomer> = Optional.ofNullable(attachExistingCustomer)
    ...
    fun asAttachExistingCustomer(): AttachExistingCustomer = attachExistingCustomer.getOrThrow("attachExistingCustomer")
     ...
     * import java.util.Optional;
     *
     * Optional<String> result = customerRequest.accept(new CustomerRequest.Visitor<Optional<String>>() {
     *     @Override
     *     public Optional<String> visitAttachExistingCustomer(AttachExistingCustomer attachExistingCustomer) {
     *         return Optional.of(attachExistingCustomer.toString());
     *     }
     *
     *     // ...
     *
    ...
    fun <T> accept(visitor: Visitor<T>): T =
        when {
            attachExistingCustomer != null -> visitor.visitAttachExistingCustomer(attachExistingCustomer)
    ...
    fun validate(): CustomerRequest =
        apply {
            ...
            accept(object : Visitor<Unit> {
                override fun visitAttachExistingCustomer(attachExistingCustomer: AttachExistingCustomer) {
    ...
    internal fun validity(): Int =
        accept(object : Visitor<Int> {
            override fun visitAttachExistingCustomer(attachExistingCustomer: AttachExistingCustomer) = attachExistingCustomer.validity()
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is CustomerRequest && attachExistingCustomer == other.attachExistingCustomer && newCustomer == other.newCustomer
    }
    ...
    override fun toString(): String =
        when {
            attachExistingCustomer != null -> "CustomerRequest{attachExistingCustomer=$attachExistingCustomer}"
    ...
    companion object {
        ...
        @JvmStatic
        fun ofAttachExistingCustomer(attachExistingCustomer: AttachExistingCustomer) = CustomerRequest(attachExistingCustomer = attachExistingCustomer)

        @JvmStatic
        fun ofNewCustomer(newCustomer: NewCustomer) = CustomerRequest(newCustomer = newCustomer)
    }

    /** An interface that defines how to map each variant of [CustomerRequest] to a value of type [T]. */
    interface Visitor<out T> {
         ...
         *
         * An instance of [CustomerRequest] can contain an unknown variant if it was deserialized from data
    ...
    internal class Deserializer : BaseDeserializer<CustomerRequest>(CustomerRequest::class) {
        ...
        override fun ObjectCodec.deserialize(node: JsonNode): CustomerRequest {
          ...
          val bestMatches = sequenceOf(
                  tryDeserialize(node, jacksonTypeRef<AttachExistingCustomer>())
                      ?.let {
                          CustomerRequest(attachExistingCustomer = it, _json = json)
                      },
                  tryDeserialize(node, jacksonTypeRef<NewCustomer>())
                      ?.let {
    ...
    internal class Serializer : BaseSerializer<CustomerRequest>(CustomerRequest::class) {
        ...
        override fun serialize(value: CustomerRequest, generator: JsonGenerator, provider: SerializerProvider) {
          when {
              value.attachExistingCustomer != null -> generator.writeObject(value.attachExistingCustomer)
