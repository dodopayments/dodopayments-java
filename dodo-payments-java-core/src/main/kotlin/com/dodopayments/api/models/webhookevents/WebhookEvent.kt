...
private constructor(
     ...
     */
    fun latestAttemptedAt(): Optional<OffsetDateTime> =
        latestAttemptedAt.getOptional("latest_attempted_at")

    /**
     * @throws DodoPaymentsInvalidDataException if the JSON field has an unexpected type (e.g. if
     *   the server responded with an unexpected value).
     */
    fun request(): Optional<String> = request.getOptional("request")
    ...
    class Builder internal constructor() {
        ...
        /** Alias for calling [Builder.latestAttemptedAt] with `latestAttemptedAt.orElse(null)`. */
        fun latestAttemptedAt(latestAttemptedAt: Optional<OffsetDateTime>) =
            latestAttemptedAt(latestAttemptedAt.getOrNull())
        ...
        fun request(request: Optional<String>) = request(request.getOrNull())
        ...
        fun response(response: Optional<String>) = response(response.getOrNull())
    ...
    internal fun validity(): Int =
        (if (businessId.asKnown().isPresent) 1 else 0) +
            ...
            (if (latestAttemptedAt.asKnown().isPresent) 1 else 0) +
            (if (request.asKnown().isPresent) 1 else 0) +
