...
class CreditRolledOverWebhookEvent @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(creditRolledOverWebhookEvent: CreditRolledOverWebhookEvent) =
            apply {
     ...
     *   expected type.
     */
    fun validate(): CreditRolledOverWebhookEvent =
        apply {
            if (validated) {
              return@apply
            }
    ...
    internal fun validity(): Int =
        (if (businessId.asKnown().isPresent) 1 else 0) + (data.asKnown().getOrNull()?.validity() ?: 0) + (if (timestamp.asKnown().isPresent) 1 else 0) + type.let {
            if (it == JsonValue.from("credit.rolled_over")) 1 else 0
        }
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is CreditRolledOverWebhookEvent && businessId == other.businessId && data == other.data && timestamp == other.timestamp && type == other.type && additionalProperties == other.additionalProperties
    }
    ...
    override fun toString() = "CreditRolledOverWebhookEvent{businessId=$businessId, data=$data, timestamp=$timestamp, type=$type, additionalProperties=$additionalProperties}"
}
