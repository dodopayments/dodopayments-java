...
private constructor(
    ...
    internal fun validity(): Int =
        (categories.asKnown().getOrNull()?.validity() ?: 0) +
            (if (compoundTriggered.asKnown().isPresent) 1 else 0) +
            (decision.asKnown().getOrNull()?.validity() ?: 0) +
            (if (latencyMs.asKnown().isPresent) 1 else 0) +
            (if (normalizedApplied.asKnown().isPresent) 1 else 0) +
            (notes.asKnown().getOrNull()?.size ?: 0) +
            (if (passes.asKnown().isPresent) 1 else 0) +
            (provenance.asKnown().getOrNull()?.validity() ?: 0) +
            (if (requestId.asKnown().isPresent) 1 else 0) +
            (triggered.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0)
