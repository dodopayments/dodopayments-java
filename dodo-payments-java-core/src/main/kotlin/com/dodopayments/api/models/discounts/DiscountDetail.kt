 ...
 */
class DiscountDetail @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    private constructor(
        ...
        @JsonProperty("cycles_remaining") @ExcludeMissing cyclesRemaining: JsonField<Int> = JsonMissing.of(),
     ...
     * @throws DodoPaymentsInvalidDataException if the JSON field has an unexpected type or is unexpectedly missing or null (e.g. if the server responded with an unexpected value).
     */
    fun preserveOnPlanChange(): Boolean = preserveOnPlanChange.getRequired("preserve_on_plan_change")
    ...
    @ExcludeMissing
    fun _position(): JsonField<Int> = position
    ...
    @ExcludeMissing
    fun _timesUsed(): JsonField<Int> = timesUsed
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(discountDetail: DiscountDetail) =
            apply {
         ...
         *
         * You should usually call [Builder.position] with a well-typed [Int] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun position(position: JsonField<Int>) =
            apply {
                this.position = position
            }

        /** Whether this discount should be preserved when a subscription changes plans */
        fun preserveOnPlanChange(preserveOnPlanChange: Boolean) = preserveOnPlanChange(JsonField.of(preserveOnPlanChange))
         ...
         *
         * You should usually call [Builder.timesUsed] with a well-typed [Int] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun timesUsed(timesUsed: JsonField<Int>) =
            apply {
        ...
        /** Remaining billing cycles for this discount on this subscription (None for one-time payments) */
        fun cyclesRemaining(cyclesRemaining: Int?) = cyclesRemaining(JsonField.ofNullable(cyclesRemaining))
        ...
        fun cyclesRemaining(cyclesRemaining: Optional<Int>) = cyclesRemaining(cyclesRemaining.getOrNull())

        /**
         * Sets [Builder.cyclesRemaining] to an arbitrary JSON value.
         *
         * You should usually call [Builder.cyclesRemaining] with a well-typed [Int] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun cyclesRemaining(cyclesRemaining: JsonField<Int>) =
            apply {
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (amount.asKnown().isPresent) 1 else 0) + (if (businessId.asKnown().isPresent) 1 else 0) + (if (code.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (discountId.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (position.asKnown().isPresent) 1 else 0) + (if (preserveOnPlanChange.asKnown().isPresent) 1 else 0) + (restrictedTo.asKnown().getOrNull()?.size ?: 0) + (if (timesUsed.asKnown().isPresent) 1 else 0) + (type.asKnown().getOrNull()?.validity() ?: 0) + (if (cyclesRemaining.asKnown().isPresent) 1 else 0) + (if (expiresAt.asKnown().isPresent) 1 else 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (subscriptionCycles.asKnown().isPresent) 1 else 0) + (if (usageLimit.asKnown().isPresent) 1 else 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is DiscountDetail && amount == other.amount && businessId == other.businessId && code == other.code && createdAt == other.createdAt && discountId == other.discountId && metadata == other.metadata && position == other.position && preserveOnPlanChange == other.preserveOnPlanChange && restrictedTo == other.restrictedTo && timesUsed == other.timesUsed && type == other.type && cyclesRemaining == other.cyclesRemaining && expiresAt == other.expiresAt && name == other.name && subscriptionCycles == other.subscriptionCycles && usageLimit == other.usageLimit && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(amount, businessId, code, createdAt, discountId, metadata, position, preserveOnPlanChange, restrictedTo, timesUsed, type, cyclesRemaining, expiresAt, name, subscriptionCycles, usageLimit, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "DiscountDetail{amount=$amount, businessId=$businessId, code=$code, createdAt=$createdAt, discountId=$discountId, metadata=$metadata, position=$position, preserveOnPlanChange=$preserveOnPlanChange, restrictedTo=$restrictedTo, timesUsed=$timesUsed, type=$type, cyclesRemaining=$cyclesRemaining, expiresAt=$expiresAt, name=$name, subscriptionCycles=$subscriptionCycles, usageLimit=$usageLimit, additionalProperties=$additionalProperties}"
}
