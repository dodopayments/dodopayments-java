...
class MeterCreditEntitlementCartResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    @ExcludeMissing
    fun _meterName(): JsonField<String> = meterName
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [MeterCreditEntitlementCartResponse].
         *
    ...
    class Builder internal constructor() {
        ...
        internal fun from(meterCreditEntitlementCartResponse: MeterCreditEntitlementCartResponse) =
            apply {
                ...
                additionalProperties = meterCreditEntitlementCartResponse.additionalProperties.toMutableMap()
            }
         ...
         *
         * You should usually call [Builder.meterName] with a well-typed [String] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun meterName(meterName: JsonField<String>) =
            apply {
                this.meterName = meterName
            }

        fun meterUnitsPerCredit(meterUnitsPerCredit: String) = meterUnitsPerCredit(JsonField.of(meterUnitsPerCredit))
        ...
        fun build(): MeterCreditEntitlementCartResponse =
            MeterCreditEntitlementCartResponse(
              ...
              checkRequired(
                "meterUnitsPerCredit", meterUnitsPerCredit
              ),
              checkRequired(
                "productId", productId
              ),
     ...
     *   expected type.
     */
    fun validate(): MeterCreditEntitlementCartResponse =
        apply {
            if (validated) {
              return@apply
            }
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (creditEntitlementId.asKnown().isPresent) 1 else 0) + (if (meterId.asKnown().isPresent) 1 else 0) + (if (meterName.asKnown().isPresent) 1 else 0) + (if (meterUnitsPerCredit.asKnown().isPresent) 1 else 0) + (if (productId.asKnown().isPresent) 1 else 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is MeterCreditEntitlementCartResponse && creditEntitlementId == other.creditEntitlementId && meterId == other.meterId && meterName == other.meterName && meterUnitsPerCredit == other.meterUnitsPerCredit && productId == other.productId && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(creditEntitlementId, meterId, meterName, meterUnitsPerCredit, productId, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "MeterCreditEntitlementCartResponse{creditEntitlementId=$creditEntitlementId, meterId=$meterId, meterName=$meterName, meterUnitsPerCredit=$meterUnitsPerCredit, productId=$productId, additionalProperties=$additionalProperties}"
}
