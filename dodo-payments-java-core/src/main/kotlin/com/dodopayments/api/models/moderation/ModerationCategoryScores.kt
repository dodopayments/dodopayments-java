...
private constructor(
    ...
    internal fun validity(): Int =
        (if (childSexualExploitation.asKnown().isPresent) 1 else 0) +
            (if (defamation.asKnown().isPresent) 1 else 0) +
            (if (hate.asKnown().isPresent) 1 else 0) +
            (if (indiscriminateWeapons.asKnown().isPresent) 1 else 0) +
            (if (intellectualProperty.asKnown().isPresent) 1 else 0) +
            (if (livingArtistStyle.asKnown().isPresent) 1 else 0) +
            (if (minorCodedLanguage.asKnown().isPresent) 1 else 0) +
            (if (nonConsensualIntimateImagery.asKnown().isPresent) 1 else 0) +
            (if (nonViolentCrimes.asKnown().isPresent) 1 else 0) +
            (if (privacy.asKnown().isPresent) 1 else 0) +
            (if (promptInjection.asKnown().isPresent) 1 else 0) +
            (if (realPersonLikeness.asKnown().isPresent) 1 else 0) +
            (if (sexRelatedCrimes.asKnown().isPresent) 1 else 0) +
            (if (sexualContent.asKnown().isPresent) 1 else 0) +
            (if (specializedAdvice.asKnown().isPresent) 1 else 0) +
            (if (suicideAndSelfHarm.asKnown().isPresent) 1 else 0) +
            (if (violentCrimes.asKnown().isPresent) 1 else 0)
