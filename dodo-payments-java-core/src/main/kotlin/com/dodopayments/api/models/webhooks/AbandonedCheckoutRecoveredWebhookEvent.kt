...
class AbandonedCheckoutRecoveredWebhookEvent @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [AbandonedCheckoutRecoveredWebhookEvent].
         *
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(abandonedCheckoutRecoveredWebhookEvent: AbandonedCheckoutRecoveredWebhookEvent) =
            apply {
                ...
                additionalProperties = abandonedCheckoutRecoveredWebhookEvent.additionalProperties.toMutableMap()
            }
     ...
     *   expected type.
     */
    fun validate(): AbandonedCheckoutRecoveredWebhookEvent =
        apply {
            if (validated) {
              return@apply
            }
    ...
    internal fun validity(): Int =
        (if (businessId.asKnown().isPresent) 1 else 0) + (data.asKnown().getOrNull()?.validity() ?: 0) + (if (timestamp.asKnown().isPresent) 1 else 0) + type.let {
            if (it == JsonValue.from("abandoned_checkout.recovered")) 1 else 0
        }
    ...
    class Data @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (abandonedAt.asKnown().isPresent) 1 else 0) + (abandonmentReason.asKnown().getOrNull()?.validity() ?: 0) + (if (customerId.asKnown().isPresent) 1 else 0) + (if (paymentId.asKnown().isPresent) 1 else 0) + (status.asKnown().getOrNull()?.validity() ?: 0) + (if (recoveredPaymentId.asKnown().isPresent) 1 else 0)
        ...
        override fun equals(other: Any?): Boolean {
          ...
          return other is Data && abandonedAt == other.abandonedAt && abandonmentReason == other.abandonmentReason && customerId == other.customerId && paymentId == other.paymentId && status == other.status && recoveredPaymentId == other.recoveredPaymentId && additionalProperties == other.additionalProperties
        }

        private val hashCode: Int by lazy { Objects.hash(abandonedAt, abandonmentReason, customerId, paymentId, status, recoveredPaymentId, additionalProperties) }

        override fun hashCode(): Int = hashCode

        override fun toString() = "Data{abandonedAt=$abandonedAt, abandonmentReason=$abandonmentReason, customerId=$customerId, paymentId=$paymentId, status=$status, recoveredPaymentId=$recoveredPaymentId, additionalProperties=$additionalProperties}"
    }
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is AbandonedCheckoutRecoveredWebhookEvent && businessId == other.businessId && data == other.data && timestamp == other.timestamp && type == other.type && additionalProperties == other.additionalProperties
    }
    ...
    override fun toString() = "AbandonedCheckoutRecoveredWebhookEvent{businessId=$businessId, data=$data, timestamp=$timestamp, type=$type, additionalProperties=$additionalProperties}"
}
