...
class CreditEntitlement @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (businessId.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (name.asKnown().isPresent) 1 else 0) + (overageBehavior.asKnown().getOrNull()?.validity() ?: 0) + (if (overageEnabled.asKnown().isPresent) 1 else 0) + (if (precision.asKnown().isPresent) 1 else 0) + (if (rolloverEnabled.asKnown().isPresent) 1 else 0) + (if (unit.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0) + (currency.asKnown().getOrNull()?.validity() ?: 0) + (if (description.asKnown().isPresent) 1 else 0) + (if (expiresAfterDays.asKnown().isPresent) 1 else 0) + (if (maxRolloverCount.asKnown().isPresent) 1 else 0) + (if (overageLimit.asKnown().isPresent) 1 else 0) + (if (pricePerUnit.asKnown().isPresent) 1 else 0) + (if (rolloverPercentage.asKnown().isPresent) 1 else 0) + (if (rolloverTimeframeCount.asKnown().isPresent) 1 else 0) + (rolloverTimeframeInterval.asKnown().getOrNull()?.validity() ?: 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is CreditEntitlement && id == other.id && businessId == other.businessId && createdAt == other.createdAt && name == other.name && overageBehavior == other.overageBehavior && overageEnabled == other.overageEnabled && precision == other.precision && rolloverEnabled == other.rolloverEnabled && unit == other.unit && updatedAt == other.updatedAt && currency == other.currency && description == other.description && expiresAfterDays == other.expiresAfterDays && maxRolloverCount == other.maxRolloverCount && overageLimit == other.overageLimit && pricePerUnit == other.pricePerUnit && rolloverPercentage == other.rolloverPercentage && rolloverTimeframeCount == other.rolloverTimeframeCount && rolloverTimeframeInterval == other.rolloverTimeframeInterval && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(id, businessId, createdAt, name, overageBehavior, overageEnabled, precision, rolloverEnabled, unit, updatedAt, currency, description, expiresAfterDays, maxRolloverCount, overageLimit, pricePerUnit, rolloverPercentage, rolloverTimeframeCount, rolloverTimeframeInterval, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "CreditEntitlement{id=$id, businessId=$businessId, createdAt=$createdAt, name=$name, overageBehavior=$overageBehavior, overageEnabled=$overageEnabled, precision=$precision, rolloverEnabled=$rolloverEnabled, unit=$unit, updatedAt=$updatedAt, currency=$currency, description=$description, expiresAfterDays=$expiresAfterDays, maxRolloverCount=$maxRolloverCount, overageLimit=$overageLimit, pricePerUnit=$pricePerUnit, rolloverPercentage=$rolloverPercentage, rolloverTimeframeCount=$rolloverTimeframeCount, rolloverTimeframeInterval=$rolloverTimeframeInterval, additionalProperties=$additionalProperties}"
}
