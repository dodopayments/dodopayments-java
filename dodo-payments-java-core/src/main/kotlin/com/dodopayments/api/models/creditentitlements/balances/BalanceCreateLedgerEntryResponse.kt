...
class BalanceCreateLedgerEntryResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    private constructor(
        ...
        @JsonProperty("entry_type") @ExcludeMissing entryType: JsonField<LedgerEntryType> = JsonMissing.of(),
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [BalanceCreateLedgerEntryResponse].
         *
    ...
    class Builder internal constructor() {
        ...
        internal fun from(balanceCreateLedgerEntryResponse: BalanceCreateLedgerEntryResponse) =
            apply {
                ...
                additionalProperties = balanceCreateLedgerEntryResponse.additionalProperties.toMutableMap()
            }
         ...
         *
         * You should usually call [Builder.entryType] with a well-typed [LedgerEntryType] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun entryType(entryType: JsonField<LedgerEntryType>) =
            apply {
     ...
     *   expected type.
     */
    fun validate(): BalanceCreateLedgerEntryResponse =
        apply {
            if (validated) {
              return@apply
            }
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (amount.asKnown().isPresent) 1 else 0) + (if (balanceAfter.asKnown().isPresent) 1 else 0) + (if (balanceBefore.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (creditEntitlementId.asKnown().isPresent) 1 else 0) + (if (customerId.asKnown().isPresent) 1 else 0) + (entryType.asKnown().getOrNull()?.validity() ?: 0) + (if (isCredit.asKnown().isPresent) 1 else 0) + (if (overageAfter.asKnown().isPresent) 1 else 0) + (if (overageBefore.asKnown().isPresent) 1 else 0) + (if (grantId.asKnown().isPresent) 1 else 0) + (if (reason.asKnown().isPresent) 1 else 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is BalanceCreateLedgerEntryResponse && id == other.id && amount == other.amount && balanceAfter == other.balanceAfter && balanceBefore == other.balanceBefore && createdAt == other.createdAt && creditEntitlementId == other.creditEntitlementId && customerId == other.customerId && entryType == other.entryType && isCredit == other.isCredit && overageAfter == other.overageAfter && overageBefore == other.overageBefore && grantId == other.grantId && reason == other.reason && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(id, amount, balanceAfter, balanceBefore, createdAt, creditEntitlementId, customerId, entryType, isCredit, overageAfter, overageBefore, grantId, reason, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "BalanceCreateLedgerEntryResponse{id=$id, amount=$amount, balanceAfter=$balanceAfter, balanceBefore=$balanceBefore, createdAt=$createdAt, creditEntitlementId=$creditEntitlementId, customerId=$customerId, entryType=$entryType, isCredit=$isCredit, overageAfter=$overageAfter, overageBefore=$overageBefore, grantId=$grantId, reason=$reason, additionalProperties=$additionalProperties}"
}
