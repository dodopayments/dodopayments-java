...
private constructor(
     ...
     */
    fun failureCode(): Optional<EmailFailureCode> = failureCode.getOptional("failure_code")
    ...
    class Builder internal constructor() {
        ...
        /** Alias for calling [Builder.failureCode] with `failureCode.orElse(null)`. */
        fun failureCode(failureCode: Optional<EmailFailureCode>) =
            failureCode(failureCode.getOrNull())
    ...
    internal fun validity(): Int =
        (if (category.asKnown().isPresent) 1 else 0) +
            ...
            (if (emailLogId.asKnown().isPresent) 1 else 0) +
            (if (emailType.asKnown().isPresent) 1 else 0) +
            (if (hasPreview.asKnown().isPresent) 1 else 0) +
            (policies.asKnown().getOrNull()?.validity() ?: 0) +
